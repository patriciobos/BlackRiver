...
# Permitir lista de columnas a graficar
...
def plot_one_csv(csv_path: Path):
    ...
    for VAL_COL in VAL_COL_LIST:
        if VAL_COL not in df.columns:
            print(f"✖ {csv_path.name}: columna '{VAL_COL}' no encontrada"); continue
        df_valid = df.dropna(subset=[LAT_COL, LON_COL, VAL_COL]).copy()
        if df_valid.empty:
            print(f"✖ {csv_path.name}: sin datos válidos para '{VAL_COL}'"); continue

        # Limpiar y crear columna tl_zmin_clean si corresponde
        if VAL_COL == "tl_zmin":
            prepare_tl_zmin(df_valid)
            vals = df_valid["tl_zmin_clean"].to_numpy(float)
        else:
            vals = df_valid[VAL_COL].to_numpy(float)
        lats = df_valid[LAT_COL].to_numpy(float)
        lons = df_valid[LON_COL].to_numpy(float)
